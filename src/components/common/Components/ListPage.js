...
const getPost = async()=>{
   ...
   .then((response)=>{
    ...
    if(listType=='upto94'){
      ...
      const item = data.filter((item)=>item.category=='GIRL-NIGHTDRESS')
      setAllItems(item)
    }
   }).catch((error)=>{
    console.log(error)
  ...
  return (
    <div className="bg-color-fff bg-fff gap-2 p-2 justify-content-center grid5 media-w-100 media-grid-2-47-47 w-90-noimpo mx-auto">
      {allItems && allItems.map((item,i)=>(
        <div key={i}  className="bg-fff box-shadow-1 hover-shadow-10 p-2 w-20 border-radius-5 d-flex flex-column justify-content-between align-items-start">
          <Link to={`/product/${item._id}`} className="text-decoration-none text-dark w-webkit">
        ...
        <div className="d-flex justify-content-center align-items-start flex-column">
          ...
          <div className="display-flex media-d-block w-100 align-items-center gap-2 mb-2">
            <StarRatting size={14} star={item.star} startCount={item.totleRatting}/>
            {item.totleRatting>200?<BadgeGreen name={"Trusted"} className={"text-light"}/>:''}
          </div>
          <div className="display-flex gap-1 align-items-center">
            ...
            <div className="font-w-400 d-flex align-items-center">
              ...
              <div className="text-decoration-line-through">₹{item.mrp}</div>
            </div>
            
          </div>
          <div className="fw-800 text-orang media-f-12 mb-2">({Number.parseInt(100-(100*item.price/item.mrp))}%OFF)</div>
          <div className="fw-600 media-f-12 f-14 line-14">Expected delivery <Moment date={addtime} className="fw-900" format="Do MM"/></div>
          <div className="media-f-12 mb-2 f-14 line-14">FREE Delivery by <b>QeenShopy</b></div>
        </div>
        </Link>
